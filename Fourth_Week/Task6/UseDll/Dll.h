...
typedef Point* (*GetP)(void);//Get the pointer
typedef void (*PutP)(const double& x, const double& y, Point& p);//Put the pointer (x, y)
typedef	void (*ShowP)(const Point& p);//Show point
typedef Line* (*GetL)(void);//Get the line
typedef void (*PutL)(const Point& a, const Point& b, Line& p);//Put the Line a, b
...
typedef Point* (*FreeP)(Point *P);//Point free
